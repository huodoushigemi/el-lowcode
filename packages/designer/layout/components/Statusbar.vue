...
import { Obj, unFn, unVal } from '@el-lowcode/utils'
...
const e = (e: StatusBarItem, { slots }) => (
  <div class={['li flex aic space-x-4']} style={e.style} op={unFn(e.disabled) && '20'} onClick={() => lcd.commands.emit(e.command!)} {...renderer(e.renderer)}>
    {e.icon && (isObject(e.icon) ? Render({ is: Icon, ...e.icon, class: normalizeClass(['hfull wa min-w22', (e.icon as Obj).class]) }) : <Icon class='hfull wa min-w22' src={unVal(e.icon)} />)}
    {e.text}
    {slots.default?.()}
  </div>
)
