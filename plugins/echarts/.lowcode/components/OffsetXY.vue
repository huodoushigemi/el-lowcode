<template>
  <svg :width="w" :height="h" @mousedown="startDrag">
    <!-- 坐标轴 -->
    <line :x1="0" :y1="centerY" :x2="w" :y2="centerY" stroke="currentColor" op20 />
    <line :x1="centerX" :y1="0" :x2="centerX" :y2="h" stroke="currentColor" op20 />
...
const startDrag = e => {
  ...
  emit('update:x', sx = e.offsetX - centerX)
  emit('update:y', sx = e.offsetX - centerX)
  
  function move(e) {
    x.value = sx + e.clientX - scx
    y.value = sy + e.clientY - scy
    emit('update:x', sx + e.clientX - scx)
    emit('update:y', sy + e.clientY - scy)
  }
