    ...
    <div ref="scrollRef" class="flex flex-col flex-1 overflow-auto scroll-smooth">
      ...
      <template v-for="msg in msgs">
        ...
        <div v-else class="p14" style="background: var(--vs-li-hover-bg)" @click="ok">
          ...
          <wc-mdit v-else="msg.content" :content="msg.content" :css="`wc-hljs { padding: 8px; ${msg.done && 'max-height: 192px;'} overflow: auto; background: #0a0a0a66; } ${!msg.done && blinkCaret}`" .options="{ highlight, linkify: true }" />
...
<script setup>
import { h, inject, ref, render, watch } from 'vue'
...
function highlight(code, lang) {
  const code2 = [
    `<div style="display: flex; justify-content: end; align-items: center; gap: 8px; padding: 4px 6px; background: #ffffff10" data-code="${encodeURIComponent(code)}">
      ...
      <button data-cb="copy" style="display: flex; align-items: center"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"><path fill="currentColor" d="M7 18V2h13v16zm2-2h9V4H9zm-6 6V6h2v14h11v2zm6-6V4z"/></svg></button>
      <button data-cb="run">Run</button>
      <button data-cb="ok">OK</button>
    </div>`.replace(/\s+/g, ' '),
    // lang != 'html' ? `<wc-hljs code="${code}" lang="${lang}"></wc-hljs>` : ''
    lang != 'html' ? `` : ''
  ].filter(e => e).join('')
  return (
    `<pre ${lang == 'html' ? 'style="margin-bottom: 0; white-space: normal;"' : ''}>${code2}</pre>` + 
    ...
    (lang == 'html' ? (() => {
      const hl = document.createElement('wc-hljs')
      hl.setAttribute('code', code)
      hl.setAttribute('lang', 'html')
      return hl.outerHTML
    })() : '')
  )
}
...
async function ok(e) {
  ...
  const map = {
    ...
    run: async () => {
      await lcd.showDialog({}, {
        default: () => h('iframe', { srcdoc: code, style: 'width: 100%; min-height: 256px', onload: 'this.height = this.contentDocument.documentElement.offsetHeight' })
      })
      map.ok()
    },
    ok: async () => {
